...
model = dict(
    ...
    decode_head=dict(
        ...
        align_corners=False,
        loss_decode=
        [dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=.5,class_weight=w),
        dict(type='DiceLoss', loss_weight=.5,class_weight=w)]
        ),
    train_cfg=dict(),
...
optimizer = dict(type='SGD', lr=0.0001 * batch_size / 10, momentum=0.9, weight_decay=0.0005)
optim_wrapper = dict(
    type='OptimWrapper',
    optimizer=optimizer,
    clip_grad=None)
