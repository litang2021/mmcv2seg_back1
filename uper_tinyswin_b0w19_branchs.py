...
data_preprocessor = dict(
    ...
    bgr_to_rgb=True,
    pad_val=0,
    seg_pad_val=255,size=crop_size)
...
model = dict(
    ...
    [dict(
        ...
        align_corners=False,
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=False, class_weight=w)),
    dict(
        type='PointHead',
        in_channels=[96],
        in_index=[0],
        ...
        loss_decode=
            [dict(
                type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0,class_weight=w),
            dict(type='JaccardLoss', use_sigmoid=False, loss_weight=1.,class_weight=w),]
...
optimizer = dict(type='SGD', lr=0.01 * batch_size / 10, momentum=0.9, weight_decay=0.0005)
optim_wrapper = dict(
    type='OptimWrapper',
    optimizer=optimizer,
    clip_grad=None)
